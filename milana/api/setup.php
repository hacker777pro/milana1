...
if ($count == 0) {
    ...
    $stmt->execute([
      ...
      $stmt->execute([
        'Помада Wet Kiss', 'DARLING', 'makeup', 790, '3.5 г',
        'Помада с эффектом увеличения губ.',
        'Наносите непосредственно из стика.',
        'Octyldodecanol, Polyethylene',
        'images/darling-wet-kiss.jpg'
    ]);
    // Тональный крем С АКЦИЕЙ (цена 999 вместо 1590)
    $stmt->execute([
        'Тональный крем Velvet Cover', 'SHIKstudio', 'makeup', 999, '30 мл',
        'Стойкий тональный крем с матовым финишем. 🔥 АКЦИЯ!',
    ...
    // Если база уже существует, обновим цену на тональный крем
    $pdo->exec("UPDATE products SET price = 999, description = 'Стойкий тональный крем с матовым финишем. 🔥 АКЦИЯ!' WHERE name LIKE '%Velvet Cover%' AND brand = 'SHIKstudio'");
    
    echo 'ℹ️ База уже существует. Товаров: ' . $count . '<br>';
    echo '✅ Акция на тональный крем применена: 999 ₽ вместо 1590 ₽';
}
?>